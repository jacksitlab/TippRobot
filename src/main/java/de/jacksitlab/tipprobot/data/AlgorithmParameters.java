...
public class AlgorithmParameters {
	...
	public String toString() {
		return String.format("%d %2.2f %2.2f %2.2f %d %2.2f: %s", this.resultCalculationParam, this.resultCalcEpsToDraw,
				this.combinedWeightFactor, this.trendDiffToWin, this.tableDiffForDraw, this.tableDividerForScore,
				this.result.toString());
	}

	public AlgorithmParameters(int p, float wf, float dtw, int dfd, float e, float dvs) {
					...
					for (int dfd : algTableDiffForDraw) {
						for (float e : tippCalcEpsToDraw) {
							for (float dvs : algTableDivForScore) {
								list.add(new AlgorithmParameters(p, wf, dtw, dfd, e, dvs));
							}
						}
					}
				}
